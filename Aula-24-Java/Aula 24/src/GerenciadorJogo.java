...
public abstract class GerenciadorJogo {
    ...
    public static void salvarJogo(Jogo jogo) {
        try (FileWriter fWriter = new FileWriter(ARQUIVO, true); BufferedWriter bWriter = new BufferedWriter(fWriter)) {
            bWriter.write(jogo + "\n");
        } catch (IOException e) {
            System.err.println("Houve um erro ao criar ou acessar o arquivo " + ARQUIVO);
        }
    }
